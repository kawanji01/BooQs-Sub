...
consumer.subscriptions.create("TranslationEliminationChannel", {
  connected() {
  ...
  received(data) {
    ...
    if ($article.length !== 0) {
      deleteTranslation(data, $article);
    }
  }
});
...
function deleteTranslation(data) {
  ...
  if (data['passage'] == null) {
    //////// タイトルの翻訳の削除 ///////
    // 編集ユーザーの削除モーダルを閉じる。
    var $form = $('#article-title-' + articleUid + '-translation-lang-' + langCode + '-edit-form-uid-' + editorToken);
    if ($form.length) {
      $('body').removeClass('modal-open');
      $('.modal-backdrop').remove();
    }
    // フィードバックラッパーの親を書き換えることで、他の編集ユーザーのフォームも含めて、他のユーザーの翻訳も削除する。
    var $translationWrapper = $('#article-title-' + articleUid + '-translation-lang-' + langCode + '-feedback-wrapper').parent();
    $translationWrapper.replaceWith(data['html']);
